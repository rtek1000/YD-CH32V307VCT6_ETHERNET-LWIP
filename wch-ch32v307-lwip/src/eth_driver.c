...
volatile uint32_t act_led_timer = 0;
#define LED_PERSISTENCE_MS 100  // Quanto tempo o LED fica aceso após um pacote
...
void Ethernet_LED_LINKSET(uint8_t setbit) {
    if (setbit) {
        GPIO_SetBits(GPIOC, GPIO_Pin_0);   // Antes era PB8
    } else {
...
void Ethernet_LED_DATASET(uint8_t setbit) {
    if (setbit) {
        GPIO_SetBits(GPIOC, GPIO_Pin_1);   // Antes era PB9
    } else {
...
void Ethernet_LED_Configuration(void) {
    ...
    // Apaga os LEDs inicialmente (Lembre-se da lógica inversa se necessário)
...
void init_phy(void) {
    ...
    //netif_set_link_callback(&lwip_netif, Ethernet_LED_LINKSET);
...
FrameTypeDef ETH_RxPkt_ChainMode(void) {
    ...
    if (frame.length > 0 && frame.length != ETH_ERROR) {
        Ethernet_LED_DATASET(0); // Liga o LED de ACT (PC1) ao receber dados válidos

        act_led_timer = LED_PERSISTENCE_MS;
    }
...
uint32_t ETH_TxPkt_ChainMode(u16 FrameLength) {
    ...
    Ethernet_LED_DATASET(0); // Liga o LED de ACT (PC1) ao enviar dados
